...
using Creative_Vein_Studio.Tools.Timeline_Scripts;
using UnityEngine;
...
public class TimeMachineMixerBehaviour : PlayableBehaviour
{
    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        for (int i = 0; i < inputCount; i++)
        {
            ...
            ScriptPlayable<TimelineTimeMachineBehaviour> inputPlayable = (ScriptPlayable<TimelineTimeMachineBehaviour>)playable.GetInput(i);
            TimelineTimeMachineBehaviour input = inputPlayable.GetBehaviour();
			...
			if(inputWeight > 0f)
			{
				if(!input.clipExecuted)
				{
					switch(input.action)
					{
						case TimelineTimeMachineBehaviour.TimeMachineAction.Pause:
						...
						case TimelineTimeMachineBehaviour.TimeMachineAction.JumpToTime:
						case TimelineTimeMachineBehaviour.TimeMachineAction.JumpToMarker:
							if(input.ConditionMet())
							{
								//Rewind
								if(input.action == TimelineTimeMachineBehaviour.TimeMachineAction.JumpToTime)
								{
									//Jump to time
